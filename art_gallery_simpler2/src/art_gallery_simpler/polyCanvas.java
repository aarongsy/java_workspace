...
class polyCanvas extends Canvas {
    ...
    public double[] evtx = new double[]{0,200,200};
    public double[] evty = new double[]{0,0,200};
    ...
    public boolean handleEvent(Event evt)
    {
        	...
        	//如果按下鼠标并且多面体不闭合
        {
            if (prevY != -1) // not the first point
            {
                ...
                if (!((Math.abs(first.getX() -evtx[evtpointer])==0 && (Math.abs(first.getY() - evty[evtpointer]) == 0))) && p.size <2000)
